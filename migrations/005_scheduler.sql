-- Schéma scheduler dans memory.db (P11 — fusion scheduler.db → memory.db)
-- Inspiré de yutoclaw (internal/jobs/job_store.go)
-- delivery_mode étendu : 'silent' | 'announce' | 'direct'
-- Mode 'direct' : envoie prompt directement à delivery_target sans appel Claude
...
CREATE TABLE IF NOT EXISTS scheduled_jobs (
  ...
  prompt TEXT NOT NULL,             -- prompt envoyé à claude CLI (ou texte direct si delivery_mode='direct')
  project_dir TEXT DEFAULT '',      -- répertoire de travail (défaut: racine projet)
  delivery_mode TEXT NOT NULL DEFAULT 'silent' CHECK(delivery_mode IN ('announce', 'silent', 'direct')),
